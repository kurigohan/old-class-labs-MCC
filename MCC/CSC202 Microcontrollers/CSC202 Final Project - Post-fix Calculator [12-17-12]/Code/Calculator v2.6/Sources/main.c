...
#include "C:\Users\Owner\Desktop\Calc\CalculatorTest - Clean\include\Calc.h"
...
Flag * createFlag(){
  ...
  f->rep = true;    //allow or prohibit operator replacing
  f->error = false;
...
void shiftMode(char key, Flag *flag){
 ...
 void addOper(char c, Flag *flag){
   if( oidx+kidx+1 < 32){  //make sure oper does not exceed its size
  ...
  if(flag->olock == false){      
    ...
    //--------------------
      ...
      if(inToPost(oper, postfix)){
        ...
        if(flag->error == false) {
          (void) sprintf(kbuf, "%.8G", calculate(postfix, flag));
          set_lcd_addr(0x40);
          type_lcd(kbuf);
          kidx = strlen(kbuf);
         } 
